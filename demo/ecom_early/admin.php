	...
	<?php 
		if(isset($_POST['upload'])){ // if upload button is clicked then proceed
			if ($_FILES['file']['error'] > 0) { // if error > 0 then do display error
			    ...
			    $image_name = mysqli_real_escape_string($_FILES['file']['name']); // store image name 
                            $pn= mysqli_real_escape_string($_POST['naam']);
                             $pc=mysqli_real_escape_string($_POST['cate']);
				$p=mysqli_real_escape_string($_POST['pr']);
				...
				&& $imageFileType != "gif" ) {
					...
					$target_file = "images/".$new_name; // unique id function make sure all images are unique
					 if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
				        ...
				        $sql = "INSERT INTO images (image_name,pname,pcat,price)
